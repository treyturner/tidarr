...
export function generateNewznabItem(
  ...
  const releaseTitle = `${formattedArtist} - ${formattedTitle}${year ? ` (${year})` : ""}`;
  const explicitTag =
    options.includeExplicitTags && album.explicit ? " [EXPLICIT]" : "";
  const tracksInfo = album.numberOfTracks
    ? ` (${album.numberOfTracks} tracks)`
    : "";
  const titleWithQuality = `${releaseTitle}${explicitTag} ${qualityInfo.qualityName} [WEB]-Tidarr${tracksInfo}`;
  ...
  return `    <item>
      ...
      <description>${escapeXml(`${releaseTitle}${explicitTag} [${qualityInfo.qualityName}] - ${qualityInfo.format} - ${album.numberOfTracks} tracks - type: ${album.type?.toUpperCase()}`)}</description>
